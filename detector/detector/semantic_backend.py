...
class _BackendDefinition:
    default_model: str
    resolve_model: Callable[[str, bool], str]
...
def _validate_gliner_config(config_path: Path, model: str) -> None:
    try:
        ...
        raise ValueError(
            f"DETECTOR_MODEL {model!r} has an unreadable {_GLINER_CONFIG}: {exc}"
        ) from exc
    if not isinstance(config, dict):
        raise ValueError(
            f"DETECTOR_MODEL {model!r} has an invalid {_GLINER_CONFIG}: "
            "the top-level value must be an object"
        )


def _validate_local_gliner_model(path: Path, configured_value: str) -> str:
    if not path.is_dir():
        raise ValueError(f"DETECTOR_MODEL local path is not a directory: {configured_value}")

    config_path = path / _GLINER_CONFIG
    if not config_path.is_file():
        raise ValueError(
            f"DETECTOR_MODEL is not a complete GLiNER checkpoint: "
            f"missing {_GLINER_CONFIG} in {configured_value}"
        )
    _validate_gliner_config(config_path, configured_value)
...
def _validate_remote_gliner_model(model: str) -> None:
    try:
        config_path = Path(hf_hub_download(model, _GLINER_CONFIG))
    except RemoteEntryNotFoundError:
        raise ValueError(
            f"DETECTOR_MODEL {model!r} is not a GLiNER checkpoint: missing {_GLINER_CONFIG}"
        ) from None
    except RepositoryNotFoundError:
        raise ValueError(
            f"DETECTOR_MODEL {model!r} was not found or is not accessible; "
            "check the Hugging Face model ID and authentication"
        ) from None
    except LocalEntryNotFoundError:
        raise ValueError(
            f"DETECTOR_MODEL {model!r} could not be validated because "
            f"{_GLINER_CONFIG} is not cached and Hugging Face is unavailable"
        ) from None
    except HfHubHTTPError as exc:
        raise ValueError(
            f"DETECTOR_MODEL {model!r} could not be validated with Hugging Face: {exc}"
        ) from exc

    _validate_gliner_config(config_path, model)


def _resolve_gliner_model(configured_value: str, is_default: bool = False) -> str:
    value = configured_value.strip()
    if not value:
        raise ValueError("DETECTOR_MODEL must not be blank")

    path = Path(value).expanduser()
    if path.exists():
        return _validate_local_gliner_model(path, configured_value)
    if _looks_like_local_path(value):
        raise ValueError(f"DETECTOR_MODEL local path does not exist: {configured_value}")

    try:
        validate_repo_id(value)
    except HFValidationError:
        raise ValueError(
            f"DETECTOR_MODEL {configured_value!r} is neither an existing local "
    ...
    if not is_default:
        _validate_remote_gliner_model(value)
    return value
