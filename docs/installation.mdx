...
default. No Hugging Face access is needed for the standard setup.

You can select a validated Hugging Face model ID or mount a local GLiNER
checkpoint. Custom Hub models need online access for the first download because
the published image deliberately starts in offline mode.

See [Semantic Backends](/configuration/semantic-backends) for the common
selection behavior and [GLiNER](/configuration/gliner) for complete Docker,
local-checkpoint, cache, and tuning examples.
...
<CardGroup cols={2}>
  ...
  <Card title="GLiNER" icon="scan-search" href="/configuration/gliner">
