...
Names and locations are multilingual. IBAN, credit card, phone, email, and IP are international. The structured identifiers (IBAN, credit card, IP) are checksum- or format-validated, so they are caught with high precision and without false positives on lookalike numbers.
