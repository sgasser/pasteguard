...
The detector always runs deterministic checks for structured identifiers and
uses a semantic backend for names, locations, and street addresses. GLiNER is
the current default and only enabled backend.

- [Semantic Backends](/configuration/semantic-backends) explains selection,
  startup validation, and backend health metadata.
- [GLiNER](/configuration/gliner) covers custom Hugging Face models, local
  checkpoints, confidence floors, and offline deployment.
...
GLiNER deployments can tune the backend floors with
`GLINER_FLOOR_PERSON`, `GLINER_FLOOR_LOCATION`, and
`GLINER_FLOOR_ADDRESS`. See [GLiNER configuration](/configuration/gliner) for
defaults, precedence, and validation rules.
