...
Configure the Anthropic endpoint for `/anthropic/v1/*` requests. Supports **mask mode only** (route mode uses OpenAI endpoint).
...
PasteGuard supports multiple authentication methods for Anthropic:

1. **Client header** — Forward client's `x-api-key` header
2. **Config API key** — Use `api_key` from config as fallback
3. **Claude subscription** — Automatically use Claude Code OAuth tokens if available

<Note>
Users with a Claude Pro/Team subscription can use PasteGuard without an API key when running locally. PasteGuard detects Claude Code OAuth tokens and uses them automatically.
</Note>
