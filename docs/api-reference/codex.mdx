...
First make sure Codex is authenticated:

```bash
codex login
```

Then add a custom Codex model provider to `~/.codex/config.toml`:
...
If PasteGuard runs on a different port, update the `base_url`.
