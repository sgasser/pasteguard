...
Names and locations are detected by the multilingual model (verified across EN/DE/IT/FR/ES/NL/PT and more). IBAN, credit card, phone, email, and IP work internationally. The structured identifiers (IBAN, credit card, IP) are checksum- or format-validated, so they are caught with high precision and without false positives on lookalike numbers.
