...
Each backend can apply its own calibrated confidence floors. For GLiNER defaults
and tuning, see [GLiNER configuration](/configuration/gliner).

See [Semantic Backends](/configuration/semantic-backends) for backend selection,
model identity, and startup behavior.
